...
void XMLInterp4Config::ProcessCubeAttrs(const xercesc::Attributes  &rAttrs)
{
  ...
  for (int i = 0; i < len; ++i) {
      ...
      else if (strcmp(attrName, "Scale") == 0){
        cube.setScale(IStrm);
      }
      else if (strcmp(attrName, "RotXYZ_deg") == 0){
        cube.setRot(IStrm);
      }
      else if (strcmp(attrName, "Trans_m") == 0){
        cube.setTransM(IStrm);
      }
      else if (strcmp(attrName, "RGB") == 0){
        cube.setRGB(IStrm);
      }
      else {
          cerr << "Nieznany atrybut \"" << attrName << "\" w <Cube>" << endl;
          exit(1);
      }
